...
def test_character_delete(complete_character):
    experience_cost = ExperienceCost(
        character_id=complete_character.id,
        type="characteristic",
        cost=10,
        name="Intelligence",
    )
...
@pytest.mark.character
def test_character_transfer(complete_character):
    old_user = User()
    ...
    complete_character.user_id = old_user.id
    assert complete_character.user == old_user
    new_user = User()
    ...
    assert response.status_code == 302
    assert response.location == "/"
    assert complete_character.user_id == new_user.id
